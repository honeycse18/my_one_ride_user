...
class SelectPaymentScreenController extends GetxController {
  ...
  Future<void> paymentAcceptCarRentRequest() async {
    final Map<String, String> requestBody = {
      '_id': postRentDetails.id,
      'method': selectedPaymentOption.value,
    };
    RawAPIResponse? response = await APIRepo.paymentCarRentRequest(requestBody);
    if (response == null) {
...
/*   _onSuccessAcceptCarRentRequest(RawAPIResponse response) {
  ...
  void _getScreenParameters() {
    ...
    if (argument is PostRentDetailsItem) {
      postRentDetails = argument;
      update();
    }
  }
